...
(defmacro preact-method
  "Internal helper macro to avoid code duplication.
   Like 'fn', but wraps the body in a binding of potato.component/*component*"
  [args & body]
  `(fn ~args
     (cljs.core/this-as this#
       (binding [potato.component/*component* this#]
         ~@body))))
...
(defmacro defcomponent
  "Creates a PreactJS component with the given name, a docstring (optional), any number of
  option->value pairs (optional), an argument vector and any number of forms body, which will be
  used as the rendering function to potato.component/component.
  For example:
    (defcomponent Widget
      'A Widget'
      :keyfn #(...)
  ...
  (let [[docstr forms] (extract-docstr forms)
    ...
    `(def ~name ~docstr (potato.component/component (fn ~argvec ~@body) ~options))))
